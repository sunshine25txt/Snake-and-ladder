...
bool hover_exit = false; // ABOUT hover হিসেবে ব্যবহার করব
...
/* UPDATED: WELCOME with ABOUT button */
void welcome_draw()
{
   ...
   center_text_draw("Debug Your Path, Climb Your Ladders, Avoid The Snakes",
                    width / 2, welcome_box.y + 120, colors.dark, medium_font);
...
/* RULES – fixed BACK button coordinates to match click handler */
void rules_draw()
{
   vector<string> lines = {
       "HOW TO PLAY:",
   ...
   // BACK button at fixed screen position matching mouse handler
...
void about_draw()
{
   ...
   center_text_draw("Version 1.0 | November 2025", width / 2, box.y + 90, colors.dark, small_font);

   int yy = box.y + 140;

   center_text_draw("A modern educational take on the classic Snakes & Ladders board game.",
                    width / 2, yy, colors.dark, medium_font);
   yy += 32;
   ...
   center_text_draw("© 2025 Koushik • Bintu • Aditya. All rights reserved.",
                    width / 2, box.y + box.h - 20, colors.dark, small_font);
}
...
void mouse_click_handle(int x, int y)
{
   ...
   // Global fallback RULES button (optional)
...
int main(int argc, char **argv)
{
   ...
   if (!xirod_font)
      fprintf(stderr, "Warning: xirod_font failed to load: %s\n", TTF_GetError());
   ...
   while (!quit)
   {
      ...
      // Footer credit (optional, only if font loaded)
      if (TTF_WasInit() && renderer)
      {
         // for safety – only if xirod_font is valid
         // We cannot call TTF_WasInit on specific font, but check pointer
      }

      if (TTF_WasInit() && renderer)
      {
         // Just to keep place if later you want to draw "POWERED BY ADITYA" each frame
      }
   ...
   // xirod_font local, but close if used
   // (we cannot refer to it here because it's local – if তুমি চাইলে উপরে global করে close করতে পারো)
