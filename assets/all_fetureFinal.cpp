...
void text_draw(const string &text, int x, int y, SDL_Color color, TTF_Font *font = nullptr)
{
   ...
   SDL_Rect dst = {x, y, surf->w, surf->h}; // where text show
                 ...
                 bool cursor_on_button = false, TTF_Font *font = nullptr)
{
   ...
   rounded_rect_draw(rect, color, 12); // ACTUAL FIX
...
void circle_draw(int cx, int cy, int r, SDL_Color c) // cx,cy=center point
{
...
void filled_eclipse_draw(int cx, int cy, int rx, int ry, SDL_Color col)
{
   ...
   for (int y = -ry; y <= ry; ++y)
   {
      ...
      int w = (int)(rx * sqrt(max(0.0f, t))); // for remove crash
      SDL_RenderDrawLine(renderer, cx - w, cy + y, cx + w, cy + y);
   }
}
...
void background_draw()
{
   for (int y = 0; y < height; y++)
   {
      ...
      SDL_SetRenderDrawColor(renderer, colors.bg1.r + (colors.bg2.r - colors.bg1.r) * t,
                             colors.bg1.g + (colors.bg2.g - colors.bg1.g) * t,
                             colors.bg1.b + (colors.bg2.b - colors.bg1.b) * t, 255);
...
void board_draw()
{
   ...
   SDL_Rect boardRect{board_left, board_top, BOARD_SIZE, BOARD_SIZE}; // border bg
   ...
   for (int i = 1; i <= 100; ++i)
   {
      ...
      if (even)
         SDL_SetRenderDrawColor(renderer, 230, 245, 255, 255); // Light blue
      else
         SDL_SetRenderDrawColor(renderer, 255, 240, 245, 255); // Light pink
      SDL_RenderFillRect(renderer, &r);
      SDL_SetRenderDrawColor(renderer, 120, 100, 160, 255); // cell border
...
void ladder_draw(int startCell, int endCell)
{
   ...
   int bx = b.x + CELL / 2, by = b.y + CELL / 2; // cell middle to ladder
   double vx = bx - ax, vy = by - ay;
   double len = hypot(vx, vy); // vector length(sqrt(x² + y²))
   if (len <= 0.0)
      return;
   vx /= len;
   vy /= len;
   ...
   {
      double t = (double)i / (double)rungs;
      int rx1 = (int)lerpf((float)ax1, (float)bx1, (float)t);
      int ry1 = (int)lerpf((float)ay1, (float)by1, (float)t);
      int rx2 = (int)lerpf((float)ax2, (float)bx2, (float)t);
      int ry2 = (int)lerpf((float)ay2, (float)by2, (float)t);
...
void realistic_snake_draw(int startCell, int endCell, const snake_design &style, float timeSec, bool highlight = false)
{
   ...
   float flick = (sin(timeSec * 12.0f) > 0.6f) ? 1.0f : 0.0f; // tongue draw
   if (flick > 0.5f)
   {
...
void question_box_draw()
{
   ...
   // Wrap question to multiple lines (simple)
   vector<string> lines;
   string w = "";
   string cur = "";
   ...
   SDL_Rect optionA = {upper_panel.x + 10, upper_panel.y + 130, 120, 30}; // option A
   ...
   SDL_Rect optionB = {upper_panel.x + 150, upper_panel.y + 130, 120, 30}; // option B
...
void rounded_rect_draw(SDL_Rect rect, SDL_Color col, int radius)
{
   ...
   // Draw rounded corners (quarter circles)
...
void welcome_draw()
{
   ...
   SDL_Rect shadow = {welcome_box.x + 8, welcome_box.y + 8, welcome_box.w, welcome_box.h};
   // SDL_SetRenderDrawColor(renderer, 0, 0, 0, 100);
   //  SDL_RenderFillRect(renderer, &shadow);
   //  rounded_rect_draw(shadow, SDL_Color{0, 0, 0, 100}, 25);

   SDL_SetRenderDrawColor(renderer, colors.white.r, colors.white.g, colors.white.b, 250); // bg
   rounded_rect_draw(welcome_box, colors.white, 25);                                      // 25px radius

   center_text_draw("THE SNADDER GAME", width / 2, welcome_box.y + 50, colors.bg2, gigantic_font);

   center_text_draw("Debug Your Path, Climb Your Ladders, Avoid The Snakes", width / 2, welcome_box.y + 120, colors.dark, medium_font);
   SDL_Rect startBtn = {width / 2 - 100, welcome_box.y + 180, 200, 60};
   button_draw(startBtn, "TAP TO START", colors.green, colors.white, false, large_font);

   SDL_Rect rulesBtn = {width / 2 - 100, welcome_box.y + 250, 200, 50};
   button_draw(rulesBtn, "RULES", colors.bg1, colors.white, false, medium_font);

   SDL_Rect settingsBtn = {width / 2 - 100, welcome_box.y + 310, 200, 50};
   button_draw(settingsBtn, "SETTINGS", colors.bg2, colors.white, false, medium_font);

   SDL_Rect exitBtn = {width / 2 - 100, welcome_box.y + 370, 200, 40};
   button_draw(exitBtn, "EXIT", colors.p1, colors.white, false, medium_font);
   button_draw(startBtn, "TAP TO START", colors.green, colors.white, hover_start);
   button_draw(rulesBtn, "RULES", colors.bg1, colors.white, hover_rules);
   button_draw(settingsBtn, "SETTINGS", colors.bg2, colors.white, hover_settings);
   button_draw(exitBtn, "EXIT", colors.p1, colors.white, hover_exit);
}
...
void name_input_box()
{
   ...
   SDL_SetRenderDrawColor(renderer, 0, 0, 0, 100);
   SDL_RenderFillRect(renderer, &shadow);
   SDL_SetRenderDrawColor(renderer, colors.white.r, colors.white.g, colors.white.b, 240); // bg
   SDL_RenderFillRect(renderer, &input_box);
   SDL_SetRenderDrawColor(renderer, colors.dark.r, colors.dark.g, colors.dark.b, 255); // border
...
void dice_draw(SDL_Rect dice_rect)
{
   ...
   SDL_Rect animatedRect = {dice_rect.x + animOffset, dice_rect.y + animOffset, dice_rect.w, dice_rect.h}; // bg
   for (int i = 0; i < animatedRect.h; i++)                                                                // 3D effect
   {
   ...
   SDL_SetRenderDrawColor(renderer, 0, 0, 0, 200); // border
   for (int i = 0; i < 3; i++)
   {
   ...
   if (game.cursor_on_dice && !game.rolling && !game.moving && game.questionState == NO_QUESTION && !game.preGameTimerActive)
   {
...
void players_draw()
{
   for (int i = 0; i < 2; i++)
   {
      ...
      center_text_draw(to_string(i + 1), cx, cy, colors.white, medium_font); // player number
   }
}
...
void UI_draw()
{
   ...
   string turnText = (game.players[game.current].name.empty() ? ("Player " + to_string(game.current + 1)) : game.players[game.current].name) + "'s Turn"; // current player indicator
   ...
   dice_draw(dice_rect);
   if (!game.rolling && !game.moving && game.questionState == NO_QUESTION) // roll instruction
   {
   ...
   for (int i = 0; i < 2; i++)
   {
      ...
      text_draw(score, lower_panel.x + 20, lower_panel.y + 250 + i * 30, game.players[i].color, medium_font);
   }
   center_text_draw("Game Stats:", lower_panel.x + lower_panel.w / 2, lower_panel.y + 320, colors.white, medium_font);
   string diceText = "Last Roll: " + to_string(game.dice);
   center_text_draw(diceText, lower_panel.x + lower_panel.w / 2, lower_panel.y + 350, colors.white, small_font);
}
void rules_draw()
{
   ...
   // Calculate auto height based on text
   int totalHeight = (int)lines.size() * lineHeight + topPadding + bottomPadding;
   ...
   // SCROLL LIMIT
   rulesScrollMax = max(0, totalHeight - box.h);
   rulesScroll = max(0, min(rulesScroll, rulesScrollMax));

   // Draw Text (With Scrolling)
   int y = box.y + topPadding - rulesScroll;
   ...
   // BACK BUTTON (always bottom visible)
   SDL_Rect backBtn = {width / 2 - 150, box.y + box.h - 70, 300, 50};
   ...
   {
      int barWidth = 12;
      int trackX = box.x + box.w - barWidth - 5;
      int trackY = box.y + 20;
      int trackH = box.h - 40;
      ...
      // Bar size proportional
      float ratio = (float)box.h / (box.h + rulesScrollMax);
      int barH = max((int)(trackH * ratio), 30);

      float posRatio = (float)rulesScroll / rulesScrollMax;
...
void settings_draw()
{
   ...
   SDL_Rect qTimeBtn = {bx, by + 120, 400, 50};
   SDL_Rect preBtn = {bx, by + 180, 400, 50};
   SDL_Rect backBtn = {bx, by + 260, 400, 50};
   ...
   button_draw(qTimeBtn,
               "Question Timer: " + to_string(game.questionDefaultTime) + " sec",
               colors.bg1, colors.white, false, medium_font);
...
void q_ans_handle(char answer)
{
   ...
   else
   {
      ...
      if (!game.isSnake)
      {
         // CORRECT on ladder → CLIMB
         if (ladders.count(pos))
            game.players[game.current].pos = ladders[pos];
      }
      else
      {
         // CORRECT on snake → avoid slide (do nothing)
      }
...
void initGame()
{
   game.players[0] = {1, colors.p1, game.players[0].name.empty() ? "Player 1" : game.players[0].name, -15, -15};
   game.players[1] = {1, colors.p2, game.players[1].name.empty() ? "Player 2" : game.players[1].name, 15, 15};
   game.current = 0;
   game.state = PLAYING;
   game.moving = false;
   game.rolling = false;
...
void dice_update()
{
   ...
   else
   {
      ...
      {
         game.current = 1 - game.current; // Can't move, switch turns
      }
   }
}
...
void movement_update()
{
   ...
   if (SDL_GetTicks() - game.last_move > 300)
   {
      ...
      {
         int pos = game.players[game.current].pos;
         if (snakes.count(pos) || ladders.count(pos)) // Check if landed on snake or ladder
         {
            start_question_challenge(pos); // start q challenge
         }
         else
         {
            if (pos == 100)
...
void handle_name_input(const string &input)
{
   ...
   else if (input == "ENTER" || input == "CONTINUE")
   {
      if (!game.tempName.empty())
      {
         ...
         {
            // start pre-game countdown and then play
            game.preGameStartTime = SDL_GetTicks();
            game.preGameTimerActive = true;
            game.preGameCountdown = game.preGameDefaultCountdown;

            // set PLAYING so drawing can show overlay; initGame resets positions
            initGame();
            // Keep preGameTimerActive true - we will block rolling until it expires.
         }
      }
   }
...
void difficulty_select_draw()
{
   ...
   button_draw(easyBtn, "EASY", colors.green, colors.white, false, large_font);
   button_draw(mediumBtn, "MEDIUM", colors.bg1, colors.white, false, large_font);
   button_draw(hardBtn, "HARD", colors.p1, colors.white, false, large_font);
   button_draw(backBtn, "BACK", colors.dark, colors.white, false, medium_font);
}

void mouse_click_handle(int x, int y)
{
   // We'll play click sound for recognized button clicks (not for every pixel)
   auto play_click = []()
   {
   ...
   case SETTINGS:
   {
      ...
      if (isPointInRect(x, y, qTimeBtn))
      {
         play_click();
         int &v = game.questionDefaultTime;
         if (v == 10)
            v = 15;
         else if (v == 15)
            v = 20;
   ...
   case PLAYING:
   {
      ...
      SDL_Rect dice_rect = {BOARD_X + BOARD_SIZE + 130, BOARD_Y + 290, 80, 80}; // dice click
   ...
   case RULES:
   {
      SDL_Rect backBtn = {width / 2 - 150, height / 2 + 170, 300, 50};
      if (isPointInRect(x, y, backBtn))
      {
         if (game.soundEnabled && snd_click)
            Mix_PlayChannel(-1, snd_click, 0);
   ...
   // Global rules button (fallback)
   SDL_Rect rulesBtn = {width / 2 - 100, height / 2 + 140, 200, 50};
   if (isPointInRect(x, y, rulesBtn))
   {
      if (game.soundEnabled && snd_click)
         Mix_PlayChannel(-1, snd_click, 0);
...
void mouse_motion_handel(int x, int y)
{
   ...
   {
   // -----------------------------------------
   // 🔵 HOVER FOR WELCOME SCREEN BUTTONS
   // -----------------------------------------
   case WELCOME:
   {
      SDL_Rect welcome_box = {width / 2 - 300, height / 2 - 200, 600, 400};
      ...
      hover_exit = isPointInRect(x, y, exitBtn);

      break;
   }

   // -----------------------------------------
   // 🟢 GAMEPLAY HOVER (your original code)
   // -----------------------------------------
   case PLAYING:
   {
...
int main(int argc, char **argv)
{
   ...
   // Load fonts
   small_font = TTF_OpenFont("assets/font/newspaper.ttf", 16);       // questions, info text
   medium_font = TTF_OpenFont("assets/font/good_timing_bd.ttf", 22); // UI elements
   large_font = TTF_OpenFont("assets/font/Pixel_Game.ttf", 30);      // main menu buttons
   gigantic_font = TTF_OpenFont("assets/font/KnightWarrior.ttf", 70);
   // title / logo
   TTF_Font *xirod_font = TTF_OpenFont("assets/font/Xirod.ttf", 24);
   center_text_draw("POWERED BY ADITYA", width / 2, 680, colors.gold, xirod_font);

   // If any font fails, we still continue but visuals will be affected.
   if (!small_font)
   ...
   // Load sounds (place WAVs in assets/sound/). Replace filenames if you have different names.
   ...
   // Warnings if sounds missing (non-fatal)
   if (!snd_roll)
   ...
   // if (!snd_ladder)
   //    fprintf(stderr, "Warning: snd_ladder failed to load: %s\n", Mix_GetError());
   if (!snd_click)
   ...
   while (!quit)
   {
      ...
      while (SDL_PollEvent(&e))
      {
         ...
         {
            if (e.wheel.y > 0)
               rulesScroll -= rulesScrollStep; // scroll up
            if (e.wheel.y < 0)
               rulesScroll += rulesScrollStep; // scroll down
         }
      }

      // If welcome requested quit (Exit button)
      if (game.requestQuit)
      {
         quit = true;
      }
      ...
      case PLAYING:
      {
         // If pre-game countdown active, draw board + overlay countdown and skip interactions
         board_draw();
         for (auto &l : ladders)
         ...
         if (game.preGameTimerActive)
         {
            ...
            // do not progress gameplay until this expires (dice_roll checks preGameTimerActive)
         }
         break;
      }
      case WINNER:
